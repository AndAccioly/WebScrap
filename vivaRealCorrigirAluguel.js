...
puppeteer.launch({ dumpio: true }).then(async browser => {
    ...
    try {
        ...
        const { data, error } = await supabase.from('imoveis').select('*').like('imagem_url', '%pixel%').eq('origem_informacao', 3).eq("tipo_negocio", 3).limit(1500)
        ...
        for (let d = 0; d < data.length; d++) {
            ...
            let imovelInfo = await page.evaluate(async () => {
                ...
                const processar = async () => {
                    ...
                    let valor_venda = Number(document.body.getElementsByClassName('price-value-wrapper')[0].getElementsByClassName('price-info-value')[0].innerHTML.split('<')[0].replaceAll('R$', '').replaceAll('&nbsp;', '').replaceAll(' ', '').replaceAll('\"', '').replaceAll('.', ''))
                    console.log(document.body.getElementsByClassName('price-value-wrapper')[0].getElementsByClassName('price-info-value')[0].innerHTML.split('<')[0])
